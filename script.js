...
function normalizeTenantRow(row) {
  ...
  normalizedRow.arnonaHistory = normalizeArnonaHistoryEntries(row.arnonaHistory);
  ...
  return raw
    .map(item => {
      ...
      return {
        startIso,
        endIso: endIso || '',
        arnonaAmount: Number(arnonaAmount.toFixed(2))
      };
    })
    .filter(Boolean)
    .sort((a, b) => a.startIso.localeCompare(b.startIso));
...
function calculateTenantBalanceBreakdown(tenant, payments, readings, waterPrice, kvaCon, todayIso = currentIsoDate(), tenantCreditsMap = null) {
  ...
  dueMonthKeys.forEach(monthKey => {
    ...
    const agg = arnonaHistoryBreakdownMap.get(key) || {
      startIso: entry.startIso,
      endIso: entry.endIso || '',
      arnonaAmount: monthlyArnona,
      months: 0,
      amount: 0
    };
    agg.months += 1;
    agg.amount += monthlyArnona;
    arnonaHistoryBreakdownMap.set(key, agg);
  });

  let arnonaAmount = 0;
  let arnonaSource = 'tenant';
  const arnonaHistoryBreakdown = Array.from(arnonaHistoryBreakdownMap.values()).sort((a, b) => a.startIso.localeCompare(b.startIso));
  if (historyArnonaUsed) {
    arnonaSource = 'history';
...
async function exportTenantsCsv() {
  ...
  tenants.slice().sort((a, b) => Number(a.apartmentNumber || 0) - Number(b.apartmentNumber || 0)).forEach(t => {
    ...
    const arnonaHistoryJson = JSON.stringify(normalizeArnonaHistoryEntries(t.arnonaHistory));
    rows.push([
      t.apartmentNumber || '',
      t.firstName || '',
      t.lastName || '',
...
async function importTenantsCsv(text) {
  ...
  const knownHeaders = new Set([
    ...
    'arnona_history_json', 'arnona_history', 'arnonahistory',
  ...
  const arnonaHistoryIdx = findHeaderIndex(['arnona_history_json', 'arnona_history', 'arnonahistory', 'arnonaHistory'], -1);
  ...
  const mergeTenantImportData = (existingTenant, importedData, options = {}) => {
    ...
    if (Array.isArray(importedData.arnonaHistory)) merged.arnonaHistory = importedData.arnonaHistory;
  ...
  for (let i = startIdx; i < rows.length; i++) {
    ...
    const data = {
      ...
      arnonaHistory: arnonaHistoryIdx >= 0 ? normalizeArnonaHistoryEntries(cellValue(row, arnonaHistoryIdx)) : [],
  ...
  const body = report.rows.map((r, idx) => {
    ...
    return `
      <tr>
        ...
        <td><button class="btn-whatsapp-pdf" data-row-index="${idx}" data-tenant="${r.apartment}" title="שיתוף דוח">שיתוף</button></td>
        <td>${r.waterMeter || '-'}</td>
        <td>${w.startValue ?? '-'}</td>
...
function getPaymentsIncomeTenantLabel(payment, tenantMap) {
  const tenant = tenantMap?.get(payment?.tenantId);
  if (tenant) {
    const tenantName = buildTenantName(tenant);
    const apartment = tenant.apartmentNumber || '';
    if (tenantName && apartment) return `${tenantName} · ${apartment}`;
    if (tenantName) return tenantName;
    if (apartment) return apartment;
  }

  const name = String(payment?.tenantName || '').trim();
  const apartment = String(payment?.apartmentNumber || '').trim();
  if (name && apartment) return `${name} · ${apartment}`;
  if (name) return name;
  if (apartment) return apartment;
  return 'דייר לא נמצא';
}

function getPaymentsIncomeBucketLabel(accountKey) {
  if (accountKey === 'my') return accountLabel('my');
  if (accountKey === 'grandma') return accountLabel('grandma');
  return 'חשבון אחר';
}

function normalizePaymentsIncomeAccountKey(accountValue) {
  const raw = String(accountValue || '').trim();
  if (raw === 'my' || raw === 'grandma') return raw;
  return 'other';
}

function buildPaymentsIncomeRootState(monthKeys, monthTotals, monthToPayments, tenantMap, subtitle) {
  return {
    type: 'root',
    title: 'סה"כ הכנסות חודשיות',
    subtitle: subtitle || 'לחץ על עמודה כדי לראות חלוקה לפי חשבון',
    labels: monthKeys.map(getMonthlyChargesMonthLabel),
    monthKeys,
    monthToPayments,
    tenantMap,
...
function buildPaymentsIncomeShell(title, subtitle, showBack) {
  ...
  container.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:12px; margin-bottom: 12px; flex-wrap: wrap;">
      <div>
        <h3 style="margin: 0 0 6px 0;">${title}</h3>
        <div id="payments-income-subtitle" style="color:#666; font-size: 13px;">${subtitle}</div>
      </div>
      <button id="payments-income-back" type="button" style="padding: 8px 14px; border: 0; border-radius: 8px; background: #27ae60; color: white; font-weight: 700; cursor: ${showBack ? 'pointer' : 'not-allowed'}; opacity: ${showBack ? 1 : 0.55};">חזור לגרף הקודם</button>
    </div>
    <div style="position: relative; height: 320px;">
      <canvas id="payments-income-chart"></canvas>
    </div>
  `;

  const backButton = container.querySelector('#payments-income-back');
  if (!backButton) return container.querySelector('#payments-income-chart');
  if (showBack) {
    backButton.removeAttribute('disabled');
  } else {
    backButton.setAttribute('disabled', 'true');
  }
  return container.querySelector('#payments-income-chart');
}

function renderPaymentsIncomeChartView(state) {
  const canvas = buildPaymentsIncomeShell(state.title, state.subtitle, state.type !== 'root');
  if (!canvas) return;

  if (window.__paymentsIncomeChartInstance) {
    window.__paymentsIncomeChartInstance.destroy();
  }

  const ctx = canvas.getContext('2d');
  window.__paymentsIncomeChartInstance = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: state.labels,
      datasets: state.datasets
  ...
  const backButton = document.getElementById('payments-income-back');
  backButton?.addEventListener('click', () => {
    if (!window.__paymentsIncomeHistory || window.__paymentsIncomeHistory.length === 0) return;
    const previous = window.__paymentsIncomeHistory.pop();
    renderPaymentsIncomeChartView(previous);
  }, { once: true });
}

async function renderPaymentsIncomeChart(payments, tenantMap) {
  const container = document.getElementById('payments-chart-container');
  if (!container) return;

  const validPayments = (payments || []).filter(payment => {
    const iso = parseDateToIso(payment?.date || '');
    const amount = Number(payment?.amount || 0);
    return Boolean(iso) && !Number.isNaN(amount);
  });

  if (validPayments.length === 0) {
    window.__paymentsIncomeHistory = [];
    renderPaymentsIncomeChartView(buildPaymentsIncomeRootState([], [], new Map(), tenantMap, 'אין תשלומים להצגה'));
    return;
  }

  const monthToPayments = new Map();
  validPayments.forEach(payment => {
    const iso = parseDateToIso(payment?.date || '');
    if (!iso) return;
    const monthKey = iso.slice(0, 7);
    if (!monthToPayments.has(monthKey)) {
      monthToPayments.set(monthKey, []);
    }
    monthToPayments.get(monthKey).push(payment);
  });

  const monthKeys = Array.from(monthToPayments.keys()).sort();
  const monthTotals = monthKeys.map(monthKey => (monthToPayments.get(monthKey) || []).reduce((sum, payment) => sum + Number(payment?.amount || 0), 0));

  window.__paymentsIncomeHistory = [];
  renderPaymentsIncomeChartView(buildPaymentsIncomeRootState(monthKeys, monthTotals, monthToPayments, tenantMap));
}
...
async function renderPayments() {
  ...
  if (all.length === 0) {
    ...
    await renderPaymentsIncomeChart([], new Map());
    return;
  }
...
function formatMonthKeyHe(monthKey) {
  if (!/^\d{4}-\d{2}$/.test(String(monthKey || ''))) return '';
  const [yearRaw, monthRaw] = String(monthKey).split('-');
  const monthIdx = Number(monthRaw) - 1;
  if (monthIdx < 0 || monthIdx > 11) return '';
  return `${HEBREW_MONTH_NAMES[monthIdx]} ${yearRaw}`;
}
...
function shiftMonthKey(monthKey, delta) {
  ...
  let month = Number(monthRaw) + Number(delta || 0);
...
function computeTenantMonthBreakdown(tenant, monthKey, ctx) {
  ...
  let electricity = 0;
  let water = 0;
  const tenantReadings = (readings || []).filter(r => Number(r?.tenantId) === tenantId);
  tenantReadings.forEach(r => {
    const iso = parseDateToIso(r?.date || '');
    if (!iso || iso.slice(0, 7) !== monthKey) return;
    const prev = findPreviousReadingForTenantMeter(tenantReadings, r);
    if (!prev) return;
  ...
  let income = 0;
  (payments || []).forEach(p => {
    if (Number(p?.tenantId) !== tenantId) return;
    const iso = parseDateToIso(p?.date || '');
    if (!iso || iso.slice(0, 7) !== monthKey) return;
  ...
  let manualCredit = 0;
  let manualDebt = 0;
  const tenantCredits = getTenantCreditEntriesFromMap(tenantCreditsMap, tenantId);
  tenantCredits.forEach(entry => {
    const iso = parseDateToIso(entry?.dateIso || '');
    if (!iso || iso.slice(0, 7) !== monthKey) return;
    const amount = Number(entry?.amount || 0);
    if (!Number.isFinite(amount) || amount <= 0) return;
    if (entry.type === 'debt') manualDebt += amount;
    else manualCredit += amount;
  });

  const obligation = rent + arnona + electricity + water + manualDebt;
  const totalIncome = income + manualCredit;
  const total = carryover + obligation - totalIncome;
  const apartment = tenant?.apartmentNumber || '';
...
function tenantValue(field, tenant) {
  ...
  if (field === 'arnonaHistory') {
    const normalized = normalizeArnonaHistoryEntries(tenant?.arnonaHistory);
    return normalized.length ? JSON.stringify(normalized) : '';
  }
